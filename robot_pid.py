...
# ============================================================================
# PID BALANCE CONTROL CONFIGURATION - EDIT THESE VALUES TO TUNE THE SYSTEM
# ============================================================================

# PID Controller Parameters - START WITH ALL VALUES AT 0 AND GRADUALLY INCREASE
PITCH_PID_PARAMS = {
    'kp': 0.6,  # Proportional gain for pitch correction - EDIT THIS LINE
    'ki': 0.0,  # Integral gain for pitch correction - EDIT THIS LINE
    'kd': 0.1   # Derivative gain for pitch correction - EDIT THIS LINE
}

ROLL_PID_PARAMS = {
    'kp': 0.0,  # Proportional gain for roll correction - EDIT THIS LINE
    'ki': 0.0,  # Integral gain for roll correction - EDIT THIS LINE
    'kd': 0.0   # Derivative gain for roll correction - EDIT THIS LINE
}

# DEADZONE CONFIGURATION - EDIT THIS LINE TO CHANGE THE DEADZONE
BALANCE_DEADZONE = 5.0  # Degrees - no correction applied within this range from calibrated position
...
def calibrate_mpu():
    ...
    set_pose(stand_angles.copy(), transition_time=1.0)
...
def get_balance_corrections():
    ...
    # PITCH CORRECTIONS (based on your specifications)
    if pitch_correction != 0:
        if pitch_error < 0:  # Negative pitch
            corrections[1] = -pitch_correction  # FL thigh -> -
            corrections[2] = pitch_correction   # FL knee -> +
            corrections[5] = pitch_correction   # FR thigh -> +
            corrections[6] = -pitch_correction  # FR knee -> -
            corrections[9] = pitch_correction   # RL thigh -> +
            corrections[10] = -pitch_correction # RL knee -> -
            corrections[13] = -pitch_correction # RR thigh -> -
            corrections[14] = pitch_correction  # RR knee -> +
        else:  # Positive pitch
            corrections[1] = -pitch_correction   # FL thigh -> +
            corrections[2] = pitch_correction  # FL knee -> -
            corrections[5] = pitch_correction  # FR thigh -> -
    ...
    # ROLL CORRECTIONS (based on your specifications)
...
def continuous_balance_monitor():
    ...
    Applies balance corrections for standing position
    """
    global continuous_balance_active
    ...
    while continuous_balance_active:
        try:
            with balance_lock:
                # Get base angles for standing position
                ...
                if corrections:
                    ...
                    if update_count % 10 == 0:
                        print(f"⚖️ [BALANCE] Pitch={current_pitch:.1f}° (err={pitch_error:.1f}°), Roll={current_roll:.1f}° (err={roll_error:.1f}°)")
...
def start_continuous_balance():
    """Start continuous balance monitoring for standing position"""
    global continuous_balance_active, continuous_balance_thread
...
def show_mpu_readings():
    """Continuously display MPU-6050 readings"""
    if mpu is None:
        print("❌ MPU6050 not available")
        return
    
    print("📊 MPU-6050 Real-time Readings")
    ...
    try:
        ...
        while True:
            ...
            # Calculate errors if calibrated
            if calibration_data['calibrated']:
                pitch_error = current_pitch - calibration_data['pitch_offset']
                roll_error = current_roll - calibration_data['roll_offset']
                
                # Check if within deadzone
                pitch_status = "✅ OK" if abs(pitch_error) < BALANCE_DEADZONE else "⚖️ TILT"
                roll_status = "✅ OK" if abs(roll_error) < BALANCE_DEADZONE else "⚖️ TILT"
                
                print(f"Raw    -> Pitch: {current_pitch:+7.2f}° | Roll: {current_roll:+7.2f}°")
                print(f"Error  -> Pitch: {pitch_error:+7.2f}° ({pitch_status}) | Roll: {roll_error:+7.2f}° ({roll_status})")
                print(f"Offset -> Pitch: {calibration_data['pitch_offset']:+7.2f}° | Roll: {calibration_data['roll_offset']:+7.2f}°")
            else:
                print(f"Raw    -> Pitch: {current_pitch:+7.2f}° | Roll: {current_roll:+7.2f}°")
                print("⚠️ Not calibrated - no error calculations available")
            
            print(f"Deadzone: ±{BALANCE_DEADZONE}° | Update: {update_count + 1}")
            print("-" * 60)
...
def walk_trot(cycles=50, enable_obstacle_avoidance=True):
    ...
    # Stop balance monitoring during movement
    stop_continuous_balance()
...
# Main program
print("🤖 Quadruped Robot with MPU6050 Balance Control")
print("Commands: straight/stand/sit/walk/right/left/distance/calibrate/balance/stop_balance/mpu/q")
print("🔧 Balance features:")
print("   - 'calibrate': Calibrate MPU6050 when robot is in standing position")
print("   - 'balance': Test current pitch/roll readings")
print("   - 'mpu': Show real-time MPU-6050 readings")
print("   - 'stand': Automatically starts continuous balance monitoring")
print("   - 'stop_balance': Stop continuous balance monitoring")
...
print(f"   Deadzone: ±{BALANCE_DEADZONE}°")
print("💡 To tune PID values, edit lines 36-50 in this script")
...
try:
    while True:
        ...
        if cmd == 'q':
            ...
            # Ask if user wants balance control
            balance_choice = input("Enable continuous balance control? (y/n, default=y): ").strip().lower()
            if balance_choice != 'n':
                if start_continuous_balance():
                    print("✅ Stand pose set with continuous balance monitoring!")
                    ...
                    print("❌ Could not start balance control - MPU6050 not available or not calibrated")
            else:
            ...
            if mpu is None:
                ...
                if continuous_balance_active:
                    print("   Continuous Balance: 🔄 ACTIVE")
                else:
                    print("   Continuous Balance: 🛑 INACTIVE")
        elif cmd == 'stop_balance':
            stop_continuous_balance()
            # Return to standing position
            set_pose(stand_angles)
            print("Balance monitoring stopped and robot returned to standing position.")
        elif cmd == 'mpu':
            print("Starting MPU-6050 real-time readings...")
            show_mpu_readings()
        else:
            print("Invalid command. Use: straight/stand/sit/walk/right/left/distance/calibrate/balance/stop_balance/mpu/q")
finally:
    print("Shutting down...")
