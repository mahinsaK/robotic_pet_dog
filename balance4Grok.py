...
# Get pitch and roll from MPU6050 with retry mechanism
def get_orientation(max_retries=3):
    for attempt in range(max_retries):
        try:
            ...
            print(f"MPU6050 read error (attempt {attempt + 1}/{max_retries}): {e}")
            time.sleep(0.1)  # Brief pause before retry
            if attempt == max_retries - 1:
                raise OSError("Failed to read from MPU6050 after multiple attempts")
    return 0, 0  # Fallback values (should not reach here due to raise)
