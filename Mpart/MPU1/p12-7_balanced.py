...
PITCH_PID_PARAMS = {
    'kp': 0.9,  # Proportional gain for pitch correction
    'ki': 0.0,  # Integral gain for pitch correction  
    'kd': 0.0   # Derivative gain for pitch correction
}
...
BALANCE_DEADZONE = 10.0  # Degrees - no correction applied within this range from calibrated position
...
def continuous_balance_stand():
    ...
    print("✅ Standing position set. Starting continuous balance monitoring...")
    print("   (Manually tilt robot to see balance corrections in action)")
    
    try:
        update_count = 0
        while True:
            # Get current angles (make a copy to avoid modifying original)
            current_target_angles = stand_angles.copy()
            ...
            if corrections:
                ...
                for ch in current_target_angles:
                    if ch in corrections:
                        current_target_angles[ch] += corrections[ch]
                        # Ensure angles stay within servo limits
                        if servo_types[ch] == "180":
                            current_target_angles[ch] = max(0, min(180, current_target_angles[ch]))
                        else:
                            current_target_angles[ch] = max(0, min(270, current_target_angles[ch]))
                
                # Apply the corrected pose
                set_pose(current_target_angles, transition_time=0.05, steps=3, enable_balance=False)
                
                # Print status every 20 updates (about every 1 second)
                update_count += 1
                if update_count % 20 == 0:
                    print(f"⚖️ Pitch={current_pitch:.1f}° (err={pitch_error:.1f}°), Roll={current_roll:.1f}° (err={roll_error:.1f}°)")
                ...
                time.sleep(0.05)  # Small delay when no corrections needed
                
    except KeyboardInterrupt:
        print("\n🛑 Continuous balance monitoring stopped")
...
def set_pose(target_angles, transition_time=0.3, steps=15, deactivate_after_move=False, enable_balance=False):
    ...
    if enable_balance:
        ...
        if corrections:
            ...
            for ch in target_angles:
                if ch in corrections:
                    target_angles[ch] += corrections[ch]
                    # Ensure angles stay within servo limits
                    if servo_types[ch] == "180":
                        target_angles[ch] = max(0, min(180, target_angles[ch]))
                    else:
                        target_angles[ch] = max(0, min(270, target_angles[ch]))
            
            # Print balance info (only if corrections are being applied)
            if any(abs(c) > 0.1 for c in corrections.values()):
...
def walk_trot(cycles=50, enable_obstacle_avoidance=True, enable_balance=False):
    ...
    if enable_balance:
        print("🔧 Balance control enabled - robot will adjust for tilt")
        if not calibration_data['calibrated']:
            print("⚠️ Warning: MPU6050 not calibrated! Please calibrate first for optimal balance.")
    
    set_pose(stand_angles.copy(), transition_time=0.015, enable_balance=enable_balance)
        ...
        if enable_obstacle_avoidance and check_obstacle():
            ...
            while check_obstacle() and obstacle_turn_cycles < max_turn_cycles:
                ...
                for phase_idx in range(4):
                    ...
                    set_pose(angles, transition_time=0.015, enable_balance=enable_balance)
                    time.sleep(0.0025)
...
# Main program
print("🤖 Quadruped Robot with Balance Control")
...
print("   - 'stand': Choose (c)ontinuous balance for PID tuning - perfect for testing!")
print("   - All movement commands now support balance control")
...
try:
    while True:
        ...
        if cmd == 'q':
            ...
            balance_choice = input("Balance control options - (n)one/(s)ingle/(c)ontinuous, default=n: ").strip().lower()
            ...
            if balance_choice == 'c':
                ...
                if enable_balance:
                    print("Stand pose set with single balance correction applied.")
                else:
