...
class RoboEyes:
    def __init__(self):
        ...
        try:
            self.disp = ili9486(serial, width=1024, height=768, rotate=1)
        except Exception:
            self.disp = ili9486(serial, rotate=1)
        print(f"Display initialized successfully! Resolution: {self.disp.width}x{self.disp.height}")

        # Use actual display dimensions instead of hardcoded constants
        self.width = self.disp.width
        self.height = self.disp.height
        self.image = Image.new("RGB", (self.width, self.height), (255, 255, 255))
        self.draw_obj = ImageDraw.Draw(self.image)

        # Adjust eye dimensions to fit the display
        self.eye_w = 170
        self.eye_h = 170
        self.eye_spacing = 30
        self.eye_radius = 18

        # Position the eyes towards the left side of the screen
        total_eye_width = 2 * self.eye_w + self.eye_spacing
        start_x = (self.width - total_eye_width) // 2 - 10  # Move 100 pixels to the left
    ...
    def draw(self):
        self.image.paste((255, 255, 255), [0, 0, self.width, self.height])
    ...
    def _draw_eye(self, draw, pos, is_left):
        ...
        # All moods: animate eye size for blink (max->min->max)
        blink_scale = 1.0
        if self.blinking:
            blink_scale = 1.0 - 0.7 * self.blink_progress  # Shrink to 30% at max blink
        anim_eye_h = int(eye_h * blink_scale)
        anim_eye_y = eye_y + (eye_h - anim_eye_h) // 2
        draw.rounded_rectangle(
            (eye_x, anim_eye_y, eye_x + eye_w, anim_eye_y + anim_eye_h),
        ...
        if self.mood == "":
            self._draw_heart(draw, cx, cy, size + 10, color)
        elif self.mood == "excited":
            self._draw_star(draw, cx, cy, size, (0, 255, 255))
        elif self.mood == "sad":
            self._draw_sad_eye(draw, eye_x, eye_y, eye_w, eye_h, is_left)
        elif self.mood == "angry":
            self._draw_angry_eye(draw, eye_x, eye_y, eye_w, eye_h, is_left)
        elif self.mood == "happy":
            self._draw_happy_eye(draw, eye_x, y)

    def _draw_sad_eye(self, draw, x, y, eye_w, eye_h, is_left):
        if is_left:
            draw.polygon([(x, y), (x + eye_w, y), (x, y + eye_h // 2)], fill=(255, 255, 255))
        else:
            draw.polygon([(x, y), (x + eye_w, y), (x + eye_w, y + eye_h // 2)], fill=(255, 255, 255))

    def _draw_angry_eye(self, draw, x, y, eye_w, eye_h, is_left):
        if is_left:
            # Make the triangle slightly larger to fully cover the rounded red rectangle
            draw.polygon([(x-2, y-2), (x + eye_w + 2, y-2), (x + eye_w + 2, y + eye_h // 2 + 5)], fill=(255, 255, 255))
        else:
            # Make the triangle slightly larger to fully cover the rounded red rectangle  
            draw.polygon([(x-2, y-2), (x + eye_w + 2, y-2), (x-2, y + eye_h // 2 + 5)], fill=(255, 255, 255))
    ...
    def update(self):
        ...
        if self.mood == "happy":
            ...
            if now >= self.next_neutral_shift_time:
                ...
                self.next_neutral_shift_time = now + random.uniform(2, 5)
